...
export function ProductCard({ product, compact = false }: ProductCardProps) {
  const hasUrl = 'url' in product && typeof product.url === 'string' && product.url.length > 0;
  const Wrapper = hasUrl ? 'a' : 'article';

  return (
    <Wrapper
      className={clsx('product-card', compact && 'product-card-compact')}
      {...(hasUrl && 'url' in product ? { href: product.url, target: '_blank', rel: 'noopener noreferrer' } : {})}
    >
