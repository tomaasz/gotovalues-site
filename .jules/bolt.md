...
When optimizing React components with static mapping objects or dictionaries (such as `escapeMap` for string sanitization), define them outside the component's render function to prevent object reallocation on every render and preserve performance gains.
